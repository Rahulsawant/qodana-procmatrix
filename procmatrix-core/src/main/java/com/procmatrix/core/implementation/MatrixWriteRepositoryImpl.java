...
public class MatrixWriteRepositoryImpl implements MatrixWriteRepository<Long, MatrixData> {
    ...
    public MatrixData persist(MatrixData matrixData) {
        try {
            MapSqlParameterSource params = new MapSqlParameterSource().addValue("data", new SqlLobValue(matrixData.getData().getBytes()), Types.BLOB);
            ...
            logger.error(String.format(ERROR_INSERTING_MATRIX ,matrixData, e));
            return null;
        }
    }
    ...
    public boolean delete(Long id) {
        try {
            ...
            logger.error(String.format(ERROR_DELETING_MATRIX , id, e));
            return false;
        }
    }
}